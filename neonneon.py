...
def initialize_cma_es(models, ps_population_size, dfs_population_size):
    ...
    dfs_bounds = np.array(dfs_bounds)
...
def get_ps_config_space(num_layers):
    ...
    for i in range(num_layers * 2):
        ps_space.append({'name': f'layer_{i}', 'type': 'real', 'bounds': [0, 1]})
    return ps_space
...
def get_dfs_config_space(num_layers, num_models, max_path_length):
    ...
    dfs_space = []
    for i in range(num_layers * max_path_length):
        dfs_space.append({'name': f'include_layer_{i}', 'type': 'categorical', 'num_categories': 2, 'bounds': [0, 1]})
    for i in range(num_layers):
        for j in range(num_models):
            dfs_space.append({'name': f'W_{i}_{j}', 'type': 'real', 'bounds': [0, 1]})
    return dfs_space
