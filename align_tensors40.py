...
def align_embeddings_manifold(embeddings1, embeddings2, method):
    ...
    return torch.tensor(aligned_embeddings[:split_index]), torch.tensor(aligned_embeddings[split_index:])

def align_embeddings_procrustes(embeddings1, embeddings2):
    R, _ = orthogonal_procrustes(embeddings2, embeddings1)
    aligned_embeddings2 = torch.matmul(torch.tensor(embeddings2), torch.tensor(R))
    return torch.tensor(embeddings1), aligned_embeddings2

def align_embeddings_cca(embeddings1, embeddings2, max_iter=2000, tol=1e-4):
    cca = CCA(n_components=min(embeddings1.shape[1], embeddings2.shape[1]), max_iter=max_iter, tol=tol)
    cca.fit(embeddings1, embeddings2)
    aligned_embeddings1, aligned_embeddings2 = cca.transform(embeddings1, embeddings2)
    return torch.tensor(aligned_embeddings1), torch.tensor(aligned_embeddings2)
...
layers_to_merge = base_model_layers[-3:]

# Define the gradient of influence for each layer
gradient_weights = [0.2, 0.3, 0.5]  # Adjust these weights as needed

# Merge the final aligned embeddings into the last three layers
for layer, weight in zip(layers_to_merge, gradient_weights):
    layer_size = None
    for name, param in layer.named_parameters():
        if layer_size is None:
            layer_size = param.data.shape[0]
        elif param.data.shape[0] != layer_size:
            raise ValueError(f"Inconsistent layer size: {layer_size} != {param.data.shape[0]}")
        
        # Slice final_aligned_embeddings to match the shape of param.data
        sliced_embeddings = final_aligned_embeddings[:layer_size]
